...
function validateSeleniumConf(configObj) {
    ...
    for (const key of configKeys) {
        if ((checkPathes.includes(key) || checkString.includes(key)) && (!sat_utils_1.isString(configObj[key]) && !sat_utils_1.isUndefined(configObj[key]))) {
            throw new TypeError(`
...
please use BaseConf type or visit https://github.com/Simple-Automation-Testing/promod/blob/master/lib/swd/config/config.ts
        ...
        if (checkPathes.includes(key) && (!sat_utils_1.isUndefined(configObj[key]) && sat_utils_1.isString(configObj[key]) && !fs.existsSync(configObj[key]))) {
            throw new TypeError(`
